...
class BotPlayer {
  ...
  findBestMove(board, rack, difficulty = 'medium', mode = 'WWF', boardSize = 15) {
    const N = boardSize;
    ...
    if (isBoardEmpty) {
      this.generateFirstWords(rack, board, validMoves, mode, N, center);
    } else {
      const anchors = this.findAnchors(board, N);
      for (const anchor of anchors) {
        this.generateMovesAtAnchor(board, anchor, rack, validMoves, mode, N);
      ...
      for (let c = 0; c < N; c++) {
        if (board[r] && !board[r][c]) {
          ...
          for (const [nr, nc] of adj) {
            if (nr >= 0 && nr < N && nc >= 0 && nc < N && board[nr] && board[nr][nc]) {
              anchors.push({ r, c });
              break;
            }
          }
  ...
  generateFirstWords(rack, board, outMoves, mode, N = 15, center = 7) {
    ...
    const permute = (currentWord, usedIndices) => {
      if (currentWord.length >= 2 && this.dictionary.isValid(currentWord)) {
        if (!testedWords.has(currentWord)) {
          ...
          const startCol = center - Math.floor(currentWord.length / 2);
          if (startCol >= 0 && startCol + currentWord.length <= N) {
            const move = [];
            ...
            const res = this.rules.validateMove(board, move, mode, N);
            if (res.valid) {
  ...
  generateMovesAtAnchor(board, anchor, rack, outMoves, mode, N = 15) {
    const r = anchor.r;
    const c = anchor.c;
